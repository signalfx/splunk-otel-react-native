# Splunk RUM React Native iOS dSYM Upload Script

The `upload-dsyms.sh` script automatically uploads debug symbol files (dSYMs) generated during your React Native iOS app builds to Splunk RUM for crash report symbolication. The script directly uploads dSYMs without requiring the Splunk RUM CLI.
...
1. **Copy the script to your project:**
   - Copy `upload-dsyms.sh` to your Xcode project directory (for React Native apps this is typically inside your app repo, e.g. `ios/` or `scripts/`)
...
4. **Set your configuration (by either)**:
   ...
   ```bash
   # Optional override. Xcode sets DWARF_DSYM_FOLDER_PATH automatically.
   export SPLUNK_DSYM_DIRECTORY="path/to/dSYMs"
   ...
   - **NOTE:** \
      ...
      The script uses it automatically when `SPLUNK_DSYM_DIRECTORY` is not set.
...
### Method 2: Expo (Development Builds)

For Expo projects using development builds (custom native code), you can use the same Xcode Run Script Phase from Method 1.

1. **Generate the iOS project:**
   - Run `npx expo prebuild` or `npx expo run:ios`
   - Open the generated `ios` project in Xcode

2. **Add the Run Script Phase:**
   - Follow the Xcode Build Phase steps in Method 1
   - Use `DWARF_DSYM_FOLDER_PATH` or `SPLUNK_DSYM_DIRECTORY` as the dSYM input

3. **Important for prebuild workflows:**
   - Re-running `expo prebuild` can regenerate the `ios` project and remove the build phase
   - Re-apply the build phase after prebuild runs (or automate later with a config plugin)

### Method 3: EAS Build (CI/CD)

EAS Build runs in CI, so add a build hook to invoke the script after the iOS build completes. Set `SPLUNK_DSYM_DIRECTORY` to the directory that contains `.dSYM` bundles in the build environment (often a path inside the Xcode archive output).

Example `eas-build-post-build` script:

```bash
#!/usr/bin/env bash
set -euo pipefail

SCRIPT_PATH="${EAS_BUILD_APP_DIR}/path/to/upload-dsyms.sh"
DSYM_DIR="${SPLUNK_DSYM_DIRECTORY:-${EAS_BUILD_APP_DIR}}"
...
- Make the hook executable: `chmod +x eas-build-post-build`
- Configure `SPLUNK_REALM` and `SPLUNK_API_ACCESS_TOKEN` in your EAS secrets or build environment
...
| `DWARF_DSYM_FOLDER_PATH` | Xcode dSYM output path (fallback) | - |
...
5. File an issue in the [splunk-otel-react-native](https://github.com/signalfx/splunk-otel-react-native) repository
