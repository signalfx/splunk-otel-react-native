...
#   DWARF_DSYM_FOLDER_PATH      - Xcode dSYM output path (used if SPLUNK_DSYM_DIRECTORY not set)
...
ENVIRONMENT VARIABLES:
    ...
    SPLUNK_API_ACCESS_TOKEN     Splunk API access token
    SPLUNK_DSYM_DIRECTORY       Directory containing dSYMs or path to .dSYM bundle
    DWARF_DSYM_FOLDER_PATH      Xcode dSYM output path (fallback for SPLUNK_DSYM_DIRECTORY)
...
validate_dsym_input() {
    if [[ -z "$DSYM_DIRECTORY" ]]; then
        log_error "dSYM directory input not specified (use -d/--directory, SPLUNK_DSYM_DIRECTORY, or DWARF_DSYM_FOLDER_PATH)"
        echo ""
        show_usage_example
        return 1
    fi
...
load_environment_variables() {
    ...
    if [[ -z "$DSYM_DIRECTORY" ]] && [[ -n "${DWARF_DSYM_FOLDER_PATH:-}" ]]; then
        DSYM_DIRECTORY="$DWARF_DSYM_FOLDER_PATH"
        log_debug "Loaded dSYM directory from DWARF_DSYM_FOLDER_PATH: $DSYM_DIRECTORY"
    fi
...
process_directory() {
    ...
    log_debug "Traversing '$current_dir' for .dSYM directories..."
